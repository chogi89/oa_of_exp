...
#define ETA_D_GAIN  0
#define ALT_P_GAIN  0.3
#define ALT_D_GAIN  0
#define ALT_SAT     0.07

#define SIGMOID_P1  4
#define SIGMOID_P2  20
...
int main (int argc, char **argv){
	...
	while (ros::ok()){
        ...
        if(count>=5){
            ...
            alt_e = 1.5 - pose_p_z_c;
            alt_e_f = LPF(alt_e_f, alt_e, CO_FRQ_ALT);
            alt_ctrl_input = PD_controller(alt_e_f, alt_e_f_p, ALT_P_GAIN, ALT_D_GAIN);
            alt_ctrl_sat_input = Saturation(alt_ctrl_input, ALT_SAT);

            sigmoid1 = 1/(1+exp(-1*SIGMOID_P2*(eta_h_sum_f-SIGMOID_P1)));
            sigmoid2 = 1/(1+exp(SIGMOID_P2*(eta_h_sum_f-SIGMOID_P1)));
            ...
            pose_o_ez_t = pose_o_ez_c + (sigmoid1 * eta_h_ctrl_signed_input) + (sigmoid2 * of_rl_ctrl_input);
...
*/
        ...
        for(int i=0; i<(WIDTH); i++){
		    for(int j=0; j<(HEIGHT); j++){
                file_image_data << arr_gray_curr[i][j] << ", ";
		    }
		}

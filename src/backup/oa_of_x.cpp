...
int main (int argc, char **argv){
	...
	while (ros::ok()){
		...
		resize(mat_gray, mat_gray, Size(WIDTH, HEIGHT));
...
*/
        ...
        if(count>=5){
            ...
            if(d_c2t < R_CHECK){
                pose_o_ex_t = 0;
                pose_o_ey_t = 0;
                pose_o_ez_t = pose_o_ez_c + S_Psi_of;
                pose_p_x_t = pose_p_x_c + D_SET*cos(pose_o_ez_c + pose_o_ez_t);
                pose_p_y_t = pose_p_y_c + D_SET*sin(pose_o_ez_c + pose_o_ez_t);
                pose_p_z_t = 1.7;
